...
import gov.fcc.mmba.convert.GeoConvert;
...
public class JsonTransformer {
	...
	private static boolean FORMAT_GEOJSON_FLAG = false;
	...
	private static final String JSON_FRONT_RAW = "[";
	private static final String JSON_BACK_RAW = "]";
	private static final String JSON_FRONT_FORMATTED = "[\n";
	private static final String JSON_BACK_FORMATTED = "\n]";
	private static final String GEOJSON_FRONT_RAW = "{\"features\":[";
	private static final String GEOJSON_FRONT_FORMATTED = "{\n\t\"features\": [";
	private static final String GEOJSON_BACK_RAW = "],\"type\":\"FeatureCollection\"}";
	private static final String GEOJSON_BACK_FORMATTED = "],\n\t\"type\": \"FeatureCollection\"\n}";
	...
	public static void main(String[] args) {
		...
		if (args.length == 0) {
			...
			System.out.print(String.format("Would you like to format restructed json file as a geojson file (defaulted to %s) -> ", (FORMAT_GEOJSON_FLAG) ? "YES" : "NO" ));
			String geojson = scan.nextLine();
			if (!geojson.trim().isEmpty())
				FORMAT_GEOJSON_FLAG = checkYesNo(geojson);
			...
			if (args.length > 1) {
				...
				if (args.length > 3)
					CONVERT_TO_JSON_FLAG = checkYesNo(args[3]);
				if (args.length > 4)
				...
				System.out
				.println("input folder location, output folder location, delimiter(optional), concatenate json files(optional - yes/no*), format(optional - yes/no*), GEOJSON(optional - yes/no*)");
			}
			scan.close();
		}
	}
	...
	private static void convert(File source, String destination) {
		try {
			...
			File jsonDestinationFile;

			if (FORMAT_GEOJSON_FLAG) {
				jsonDestinationFile = new File(destination + File.separatorChar + "samknows." + System.currentTimeMillis() + ".geojson");	
			} else {
				jsonDestinationFile = new File(destination + File.separatorChar + "samknows." + System.currentTimeMillis() + ".json");
			}
			...
			// Handle front of JSON file
			if (CONVERT_TO_JSON_FLAG && FORMAT_GEOJSON_FLAG) {
				bwjson.write((FORMAT_JSON_FLAG) ? GEOJSON_FRONT_FORMATTED : GEOJSON_FRONT_RAW);
			} else if (CONVERT_TO_JSON_FLAG) {
				bwjson.write((FORMAT_JSON_FLAG) ? JSON_FRONT_FORMATTED : JSON_FRONT_RAW);
			}
						...
						if (jObject != null) {
							...
							if (CONVERT_TO_JSON_FLAG) {
								if (first) {
									...
									bwjson.write(JSON_COMMA);
								}

								if (FORMAT_JSON_FLAG) {
									JsonRootNode j = (FORMAT_GEOJSON_FLAG) ? JDOM_PARSER.parse(GeoConvert.getFeature(json.toString()).getAsJSONObject().toString()) : JDOM_PARSER.parse(json.toString());
									String formattedJson = JSON_FORMATTER.format(j);
									bwjson.write(formattedJson);
								} else {
									if (FORMAT_GEOJSON_FLAG) {
										bwjson.write(GeoConvert.getFeature(json).getAsJSONObject().toString());
									} else {
										bwjson.write(json);
									}
								}
			...
			// Handle back of JSON file
			if (CONVERT_TO_JSON_FLAG && FORMAT_GEOJSON_FLAG) {
				bwjson.write((FORMAT_JSON_FLAG) ? GEOJSON_BACK_FORMATTED : GEOJSON_BACK_RAW);
			} else if (CONVERT_TO_JSON_FLAG) {
				bwjson.write((FORMAT_JSON_FLAG) ? JSON_BACK_FORMATTED : JSON_BACK_RAW);
			}
